...
public class Position {
	...
	public void color(int i, int j, int c) {
		F[i][j].color(c);
	}

	public void number(int i, int j, int n) {
		F[i][j].number(n);
	}
	...
	/**
	 * Recursively mark all unmarked places with state c, starting from (i,j).
	 */
	public void markrek(int i, int j, int c) {
		if (F[i][j].mark() || F[i][j].color() != c)
			return;
		F[i][j].mark(true);
		if (i > 0)
			markrek(i - 1, j, c);
	...
	public int marker(int i, int j) {
		return F[i][j].marker();
	}

	public void marker(int i, int j, int f) {
		F[i][j].marker(f);
	}

	public void letter(int i, int j, int l) {
		F[i][j].letter(l);
	}

	public int letter(int i, int j) {
		return F[i][j].letter();
	}

	public int territory(int i, int j) {
		return F[i][j].territory();
	}

	public boolean haslabel(int i, int j) {
		return F[i][j].havelabel();
	}

	public String label(int i, int j) {
		return F[i][j].label();
	}

	public void setlabel(int i, int j, String s) {
		F[i][j].setlabel(s);
	}

	public void clearlabel(int i, int j) {
		F[i][j].clearlabel();
	}

	// Interfact to variation trees
	public TreeNode tree(int i, int j) {
		return F[i][j].tree();
	}

	public void tree(int i, int j, TreeNode t) {
		F[i][j].tree(t);
	}
}
